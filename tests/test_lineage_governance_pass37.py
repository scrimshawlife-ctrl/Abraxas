...
def test_provenance_derivation_determinism() -> None:
    ...
    assert report_a == report_b
    assert set(x["derivation_state"] for x in report_a["derivations"]).issubset({"DERIVATION_VALID", "DERIVATION_UNKNOWN", "STALE_DERIVED_STATE"})
...
def test_mutation_legitimacy_determinism() -> None:
    ...
    assert report_a == report_b
    assert set(report_a["mutationStates"].values()).issubset({"MUTATION_LEGITIMATE", "MUTATION_CONDITIONAL", "MUTATION_ILLEGITIMATE", "UNAUTHORIZED_MUTATION"})
...
def test_provenance_transition_determinism() -> None:
    ...
    assert {x["to_state"] for x in report_a["transitions"]}.issuperset(
        {"PROVENANCE_STALE", "STALE_DERIVED_STATE", "UNAUTHORIZED_MUTATION", "NON_REPLAYABLE_STATE"}
    )
...
def test_lineage_scorecard_determinism_and_blockers() -> None:
    ...
    assert score_a.category in {"LINEAGE_GOVERNED", "STALE_DERIVED_BURDENED", "PROVENANCE_BROKEN_BURDENED", "BLOCKED"}
...
def test_elegance_regressions_and_invariance_guards() -> None:
    ...
    assert len(set(mutation["mutationStates"].values())) <= 4
