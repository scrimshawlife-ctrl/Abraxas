"""
Abraxas Runtime N-Run Gate (artifact-level).

Usage:
  python -m scripts.n_run_gate_runtime --artifacts_dir ./artifacts
  python -m scripts.n_run_gate_runtime --artifacts_dir ./artifacts --run_id my_run --runs 12
...
def _gate_note(runs: int, ok: bool) -> str:
    state = "pass" if ok else "failure"
    return f"{runs}-run gate {state}"


def main() -> int:
    ap = argparse.ArgumentParser(description="Run N-run invariance gate")
    ...
    ap.add_argument("--run_id", default="n_run_gate", help="Run ID for stability record")
    args = ap.parse_args()
    if args.runs < 1:
    ...
    if not res.ok:
        print(f"{args.runs}-RUN GATE: FAIL")
        print("=" * 60)
        print("TrendPack:")
        ...
        stability_path, stability_sha = write_run_stability(
            ...
            note=_gate_note(args.runs, ok=False),
        )
        ref_path, _ = write_stability_ref(
            artifacts_dir=args.artifacts_dir,
            run_id=args.run_id,
    ...
    print(f"{args.runs}-RUN GATE: PASS")
    print("=" * 60)
    ...
    stability_path, stability_sha = write_run_stability(
        ...
        run_id=args.run_id,
        gate_result=gate_obj,
        note=_gate_note(args.runs, ok=True),
    )
    ref_path, _ = write_stability_ref(
        artifacts_dir=args.artifacts_dir,
        run_id=args.run_id,
