...
def main() -> None:
    ...
    parser.add_argument("--approval", default="fixtures/pse/calibration_candidate_approval_packet.v1.json")
    parser.add_argument("--readiness", default="out/reports/abx_readiness_gate.latest.json")
    parser.add_argument("--out", default="out/reports/pse_calibration_candidate_approval_gate.latest.json")
    args = parser.parse_args()

    proposal = json.loads((REPO_ROOT / args.proposal).read_text(encoding="utf-8"))
    approval = json.loads((REPO_ROOT / args.approval).read_text(encoding="utf-8"))
