...
def run_v11_prefix(
    ...
    *,
    ...
    post = warden_post(binding, weave, report_id=f"{run_id}_ward_post")

    bound = binding.get("status") == "BOUND_SHADOW" and post.invariance_passed
    output_hash = compute_keep_output_hash(binding, weave, post) if bound else None
    ...
    entry = keeper.run(
        ...
        meta={
            ...
            "ward_post_passed": post.invariance_passed,
            "output_hash_method": "keep_output.v1",
        },
    )
