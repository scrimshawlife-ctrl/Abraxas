...
from ..sdct.domains.numogram_motif import NumogramMotifCartridge
from ..sdct.numogram.spec_v1 import build_default_graph_v1, validate_graph
...
def _digit_signature(digits: List[str]) -> str:
...
def run(payload: Dict[str, Any], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    digit_motifs = payload.get("digit_motifs_by_item_id", {})
    if not isinstance(digit_motifs, dict):
        digit_motifs = {}

    graph = build_default_graph_v1()
    validate_graph(graph)
    cartridge = NumogramMotifCartridge(graph=graph)
    ...
    input_hash = _input_hash({
        ...
        "digit_motifs_by_item_id": digit_motifs,
    })
    provenance = {
    ...
    for item in items:
        ...
        digits = digit_motifs.get(item_id, [])
        if not isinstance(digits, list):
            digits = []
        signature = _digit_signature([str(d) for d in digits])
        sym = cartridge.encode(event_key, date, signature)
